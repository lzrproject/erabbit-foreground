...
export default {
  ...
  getters: {
    ...
    validAmount (state, getters) {
      ...
      return getters.validList.reduce((p, c) => p + Math.round(c.price * 100) * c.count, 0) / 100
    },
    // 无效商品列表
    invalidList (state) {
      return state.list.filter(goods => goods.stock <= 0 || !goods.isEffective)
    ...
    selectedAmount (state, getters) {
      return getters.selectedList.reduce((p, c) => p + Math.round(c.price * 100) * c.count, 0) / 100
    },
    // 是否全选
