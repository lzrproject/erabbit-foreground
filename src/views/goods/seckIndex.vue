    ...
    <div class="container">
      ...
      <div class="goods-info">
        ...
        <div class="spec">
          ...
          <GoodsSku :goods="goods" :skuId="goods.id" @change="changeSku"  />
          <!-- 数量选择组件 -->
          <XtxNumbox label="数量" v-model="num" :max="1" />
          <!-- 按钮组件 -->
          <XtxButton @click="insertCart()" type="primary" style="margin-top:20px">立即抢购</XtxButton>
        </div>
      </div>
...
import { findseckillGoods } from '@/api/product'
  ...
  setup () {
    ...
    const insertCart = () => {
      if (currSku.value && currSku.value.skuId) {
        ...
        // })
        console.log(goods)
        router.push(`/member/seckillCheckout/${goods.value.startTime}/${goods.value.seckillId}`)
      } else {
...
const useGoods = () => {
  ...
  watch(() => time + '/' + seckillId, (newVal) => {
    if (newVal && `/productSeckill/${newVal}` === route.path) {
      findseckillGoods(time, seckillId).then(data => {
