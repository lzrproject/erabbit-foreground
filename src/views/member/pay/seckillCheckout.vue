...
import { submitOrder, repurchaseOrder, createSeckillOrder } from '@/api/order'
  ...
  setup () {
    ...
    if (route.query.orderId) {
      ...
      repurchaseOrder(route.query.orderId).then(data => {
        order.value = data
        reqParams.goods = data.goods.map(({ skuId, count }) => ({ skuId, count }))
      })
    } else {
      // 按照购物车商品结算
      createSeckillOrder(time, seckillId).then(data => {
        order.value = data
        reqParams.goods = data.goods.map(({ skuId, count }) => ({ skuId, count }))
      })
    }
    ...
    const submitOrderFn = () => {
      ...
      submitOrder(reqParams).then(data => {
        if (data.flag) {
          ...
          // 跳转支付页面
          router.push(`/member/pay?orderId=${data.data}`)
        } else {
