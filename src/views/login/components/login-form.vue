...
import { userAccountLogin, userMobileLogin, userMobileLoginMsg, userAccountDetail } from '@/api/user'
      ...
      if (valid) {
        try {
          ...
          if (data.flag) {
            ...
            const userDetail = await userAccountDetail()
            // 存储用户信息
            const { id, account, avatar, mobile, nickname } = userDetail
            store.commit('user/setUser', { id, account, avatar, mobile, nickname })
            store.commit('user/setToken', token)
            ...
            Message({ type: 'error', text: data.message || '登录失败' })
          }
        } catch (e) {
          // console.log(e)
          // 失败提示
